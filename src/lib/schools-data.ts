...
const GVA_SCHOOLS: School[] = (rawData as RawSchoolRecord[]).map((r) => {
  ...
  return {
    ...
    buildingCondition: r.fci > 0 ? (r.fci <= 0.15 ? "Good" : r.fci <= 0.40 ? "Fair" : r.fci <= 0.60 ? "Poor" : "Critical") as "Good" | "Fair" | "Poor" | "Critical" : undefined,
    seismicRisk: r.seismicRisk === "Completed" ? "Upgrades Complete" : r.seismicRisk?.startsWith("H") ? "High" : r.seismicRisk === "M" ? "Medium" : r.seismicRisk === "L" ? "Low" : r.seismicRisk ? (r.seismicRisk as "High" | "Medium" | "Low" | "Upgrades Complete") : undefined,
